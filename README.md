...
| 檔位 | 機率門檻 | 語音含量下限 | 靜音容忍 | 送出長度下限 | 什麼時候用 |
|---|---|---|---|---|---|
| 靈敏 `sensitive`（預設） | 0.30 | 120 ms | 900 ms | 250 ms | **耳語、短應答（はい／うん）**被漏掉時 |
| 標準 `normal` | 0.50 | 1000 ms | 600 ms | 500 ms | **水流、風扇、冷氣、人聲吵雜**的環境 |
| 嚴格 `strict` | 0.70 | 1500 ms | 500 ms | 700 ms | **BGM 吵、幻聽多**時 |

**丟棄量太多？先看狀態帶寫的是哪一種「太短」。**

短句被丟有兩道關卡，以前都只寫「太短」，現在分開標示：

| 狀態帶文字 | 是誰擋的 | 怎麼調 |
|---|---|---|
| 太短（VAD） | `vad_min_speech_ms`：這段有聲時間太短，VAD 不當成一段 | 切靈敏檔 |
| 太短（未達送出長度） | `vad_min_chunk_ms`：成段了但長度不夠，不值得送去辨識 | 切靈敏檔 |
| 語音含量不足 | `vad_min_voiced_ms`：整段裡「真的是人聲」的比例太低 | 切靈敏檔；若同時噪音大，改切標準檔 |

日文對話的應答（はい／うん／ええ／そう）落在 0.3~0.6 秒，舊版靈敏檔的
`min_voiced` 400 ms 加上寫死的 0.5 秒送出門檻會把它們整批丟掉——這就是
「已丟棄 13、太短 9」的來源。現在靈敏檔的三道門檻分別降到 120 / 60 / 250 ms，
0.35 秒的應答送得出去，而機率門檻沒動，所以對 BGM 的誤判率不變（仍是 16%）。
